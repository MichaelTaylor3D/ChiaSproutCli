...
const API_ENDPOINT =
  "https://api.datalayer.storage/mirrors/v1/check_connection";
...
async function checkFilePropagationServerReachable() {
  ...
  if (ip4) {
    const successIPv4 = await checkConnection(
      `http://${ip4}:${CHIA_PORT}`,
      "IPv4"
    );
  ...
  if (ip6) {
    ...
    if (ipv6Regex.test(ip6)) {
      const successIPv6 = await checkConnection(
        `http://[${ip6}]:${CHIA_PORT}`,
        "IPv6"
      );
...
async function checkChiaConfigIpHost() {
  try {
    ...
    if (
      !config.data_layer ||
      typeof config.data_layer.host_ip === "undefined"
    ) {
    ...
    console.log(
      `Chia config data_layer.host_ip: ${config.data_layer.host_ip || '""'}`
    );
    return (
      config.data_layer.host_ip === "0.0.0.0" ||
      config.data_layer.host_ip === ""
    );
  } catch (error) {
